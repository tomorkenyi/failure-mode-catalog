...
@Entity
@NamedQueries({
        @NamedQuery(name = FailureMode.FIND_BY_ID, query = "SELECT DISTINCT fm FROM FailureMode fm LEFT JOIN FETCH fm.tags WHERE fm.id = :pid"),
        @NamedQuery(name = FailureMode.FIND_ALL, query = "SELECT DISTINCT fm FROM FailureMode fm LEFT JOIN FETCH fm.tags")
})
